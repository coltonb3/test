...
#google sheets api AIzaSyCbsomTDIsAW1BlVY9bHGhu60pcosvuFq0



API_KEY = '3ff524cbf55150ac653432f9b051ccf3'
...
class Event:
    ...
    def find_best_odds(self):
        ...
        num_outcomes = len(self.data['bookmakers']
                           [FIRST]['markets'][FIRST]['outcomes'])
        ...
        for index, bookmaker in enumerate(bookmakers):
            ...
            for outcome in range(num_outcomes):
                ...
                bookmaker_odds = float(
                    bookmaker['markets'][FIRST]['outcomes'][outcome]['price'])
...
my_columns = ['ID', 'Sport Key', 'Expected Earnings'] + list(np.array(
    [[f'Bookmaker #{outcome}', f'Name #{outcome}', f'Odds #{outcome}', f'Amount to Buy #{outcome}'] for outcome in range(1, MAX_OUTCOMES + 1)]).flatten())
...
ID_COLUMN_FILL = PatternFill(
    fill_type='solid', start_color=DARK_GREY, end_color=DARK_GREY)
SPORT_KEY_COLUMN_FILL = PatternFill(
    fill_type='solid', start_color=LIGHT_GREY, end_color=LIGHT_GREY)
EXPECTED_EARNINGS_COLUMN_FILL = PatternFill(
...
NORMAL_ROW_BORDER = Border(top=Side(border_style='thin', color=LIGHT_GREY), bottom=Side(
    border_style='thin', color=DARK_GREY))
